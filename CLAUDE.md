...
├── python.rs           # define_tokenizer! macro + the 4 #[pyclass] wrappers
├── core.rs             # TokenizerCore: shared, granularity-agnostic logic
├── pretokenizer.rs     # PreTokenizer enum (Atom vs Char splitting)
...
All four are thin `#[pyclass]` wrappers over a single shared `TokenizerCore`,
...
- **Pre-tokenization** (`pretokenizer.rs`): `PreTokenizer` does atom-level (regex) or character-level (Unicode scalar) splitting; the rest of the pipeline is granularity-agnostic
- **Shared core** (`core.rs`): `TokenizerCore` holds all train/encode/decode/pad logic; the four classes delegate to it
...
PyO3 exposes the four tokenizer classes (`CharTokenizer`, `AtomTokenizer`, `CharBPETokenizer`, `SmilesTokenizer`) and the `atomwise_tokenize()` function. Type stubs in `rustmolbpe.pyi` (a shared `_BaseTokenizer` plus four subclasses) for IDE support. The package includes `py.typed` marker for PEP 561 compliance. See the `.pyi` file for the full API surface.
...
- `tokenizer_stats.py` - Token-count statistics for all four tokenizers on ChEMBL 36 (needs the `stats` extra: `uv pip install numpy matplotlib`)
...
- The four tokenizer classes are generated by the `define_tokenizer!` macro in `python.rs`; add/change a method there once, not four times
...
- A pickle can only be restored into a tokenizer of matching granularity (atom vs char); cross-class unpickling raises `ValueError`
