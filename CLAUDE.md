...
- `ByteBPETokenizer` always seeds all 256 byte values as its base vocab (`base_vocab_size == 260`), so it never emits `UNK`; `load_vocabulary`/`save_vocabulary`/`save_huggingface`/`from_huggingface` raise `NotImplementedError` (those formats store readable tokens, not raw bytes — use `save`/`from_file`)
